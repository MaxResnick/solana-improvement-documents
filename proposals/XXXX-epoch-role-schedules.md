...
stake-weighted schedules at the start of each epoch:
...
The proposer and attester committees are rolling committees. They do not change
all at once. Each new slot replaces one proposer, and each new cycle replaces
one attester.

## Motivation

The existing leader schedule gives the cluster a deterministic slot leader, but
it does not define deterministic proposer or attester assignments. Protocol
work that separates these responsibilities needs all validators to agree on the
same role assignments during replay, restart, and snapshot restore.

The goal of this proposal is to derive those assignments from the same kind of
epoch stake snapshot already used for leader scheduling. The output should be
cheap to look up, deterministic across validators, and independent of local
wallclock time.

### Background

Today, Solana derives the leader schedule from a bank's epoch stake snapshot.
The snapshot contains the vote accounts, validator identities, and delegated
stake for the leader schedule epoch. Validators filter out zero-stake vote
accounts and use stake-weighted sampling to choose leaders.

The randomness for the current leader schedule is deterministic. The epoch
number is encoded into a 32-byte seed and used to initialize a ChaCha RNG. The
sampled leaders are then expanded into the slot schedule. In the current
implementation, one sampled leader is assigned to a consecutive window of four
slots.

The expanded leader schedule is cached in memory by epoch. It is not persisted
...
Zero-stake entries are ignored. The remaining entries form the eligible set for
sampling.

All three schedules use stake-weighted sampling with replacement over this
eligible set. The sampling randomness is generated from independent ChaCha RNG
streams. Each stream is seeded with:

```
SHA256(domain || genesis_hash || epoch_le)
```

where `epoch_le` is the epoch encoded as a little-endian `u64`. The domains
are:

```
leader-schedule-v1
proposer-schedule-v1
attester-schedule-v1
```

The domain-separated streams allow the proposer and attester schedules to
change independently of the leader schedule while still being derived from the
same epoch stake snapshot.
...
This proposal does not change the leader window duration. The current leader
window behavior is preserved, so one stake-weighted leader sample continues to
be expanded across the existing consecutive slot window.
...
After the first slot, the proposer committee advances by replacing one member
per slot:
...
This gives adjacent slots stable proposer membership while still rotating the
committee across the epoch.
...
This proposal defines the attester schedule as a function from cycle index to
attester committee. It does not require local wallclock time to be part of
schedule derivation.

If attester timing is consensus-critical, the current cycle must be derived
from deterministic ledger state, such as PoH tick height or another committed
time source. Local wallclock time may be used for non-consensus behavior, such
as estimating the current cycle for prefetching or monitoring, but it must not
decide consensus validity.
...
Given the same epoch stake snapshot, genesis hash, epoch number, and protocol
constants, every validator should compute the same lookup results.

## Alternatives Considered

This proposal only introduces deterministic, initially unused schedules.
Alternatives for how proposer and attester roles are consumed are left to
follow-up proposals.
...
RNG seeding, and cycle selection must not depend on local process state or map
iteration order.
...
The attester cycle source needs particular care. If attestations affect block
validity or fork choice, cycle selection must come from ledger state rather
than wallclock time.
...
1. Which committed source should define the cycle index when attestation timing
   is consensus-critical?
2. Should validators with multiple vote accounts be sampled at the vote-account
